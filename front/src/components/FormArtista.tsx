...
export function FormArtista({ artistaId }: FormArtistaProps) {
  ...
  const onSubmit = (fn: () => void) => {
    const submit: SubmitHandler<IFormValues> = async (data) => {
      try {
        await api.put(`artistas/${artistaId}`, data);
        Toast.fire({
          icon: "success",
  ...
  const loadData = () => {
    api.get<IFormValues>(`artistas/${artistaId}`).then((res) => {
      setValue("f_name", res.data.f_name);
      setValue("l_name", res.data.l_name);
    });
  };
